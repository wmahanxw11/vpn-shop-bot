...
def handle_buy(call):
    ...
    if assign_link_to_user(link.id, user_id):
        if deduct_balance(user_id, PRICE):
            add_transaction(user_id, -PRICE, f"خرید اشتراک")
            bot.send_message(
                user_id,
...
def handle_status(call):
    ...
    if purchased_links:
        ...
        for i, link in enumerate(purchased_links[-5:], 1):
...
def admin_charge():
    ...
    if telegram_id and amount > 0:
        try:
            ...
            try:
                bot.send_message(int(telegram_id), f"💰 کیف پول شما به مبلغ {amount} تومان شارژ شد.")
            except:
                pass
...
if __name__ == '__main__':
    ...
    # تنظیم Webhook در Railway (اختیاری)
